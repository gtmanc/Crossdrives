...
//		treeOpener.getItems().observe(this, listChangeObserver)
		...
		// An exception to this behavior is that observers also receive an update when they change
		// from an inactive to an active state. Furthermore, if the observer changes from inactive
		// to active a second time, it only receives an update if the value has changed since the last time
		// it became active. https://developer.android.com/topic/libraries/architecture/livedata#observe_livedata_objects
		treeOpener.getItems().observe(this, list -> {
